...
const CommentsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const { data } = await axios.get(`http://localhost:8000/api/v1/comment/get-comments/${id}`);
  ...
  const handleAddComment = async () => {
    try {
      const { data } = await axios.post('http://localhost:8000/api/v1/comment/add-comment', {
        text: commentText,
        user: userId,
        blog: blogId,
      });
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const { data } = await axios.delete(`http://localhost:8000/api/v1/comment/delete-comment/${commentId}`);
      if (data.success) {
