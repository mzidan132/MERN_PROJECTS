...
const CommentsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const { data } = await axios.get(`https://mern-projects-gs9u.onrender.com/api/v1/comment/get-comments/${id}`);
  ...
  const handleAddComment = async () => {
    try {
      const { data } = await axios.post('https://mern-projects-gs9u.onrender.com/api/v1/comment/add-comment', {
        text: commentText,
        user: userId,
        blog: blogId,
      });
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const { data } = await axios.delete(`https://mern-projects-gs9u.onrender.com/api/v1/comment/delete-comment/${commentId}`);
      if (data.success) {
